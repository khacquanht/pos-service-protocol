...
import com.service_wahsis.user_guid.UserGuidHandler;
 ...
 * @author khiemnv
 */
public class UserGuidServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                UserGuidHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(UserGuidServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
}
