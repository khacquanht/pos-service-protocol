...
import com.service_wahsis.core_service.SetupDataCompanyHandler;
 ...
 * @author khiemnv
 */
public class SetupDataCompanyServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                SetupDataCompanyHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(SetupDataCompanyServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
