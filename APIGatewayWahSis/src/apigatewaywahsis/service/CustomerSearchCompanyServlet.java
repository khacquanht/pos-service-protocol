...
import com.service_wahsis.customer_service.CustomerSearchCompanyHandler;
 ...
 * @author khiemnv
 */
public class CustomerSearchCompanyServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                CustomerSearchCompanyHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(CustomerSearchCompanyServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
