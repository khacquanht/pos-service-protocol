...
import com.service_wahsis.customer_service.CustomerRequestHandler;
 ...
 * @author khiemnv
 */
public class CustomerRequestServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                CustomerRequestHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(CustomerRequestServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
