...
import com.service_wahsis.company_provide_service.CompanyProvideServicePartnerHandler;
 ...
 * @author khiemnv
 */
public class CompanyProvideServicePartnerServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                CompanyProvideServicePartnerHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(CompanyProvideServicePartnerServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
}
