...
import com.service_wahsis.customer_service.CompanyCustomersHandler;
 ...
 * @author khiemnv
 */
public class CompanyCustomersServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                CompanyCustomersHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(CompanyCustomersServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
