...
import com.service_wahsis.database_service.ClusterServerHostingConfigHandler;
 ...
 * @author khiemnv
*/
public class ClusterServerHostingConfigServlet extends AbstractServlet{
	@Override
	protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
		...
		String data = req.getParameter(DefinedName.PARAM_DATA);
		 if (cmd != null && data != null)
			ClusterServerHostingConfigHandler.getInstance().process(cmd, data, respContent, vrfContent);
		 else
			respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
		} catch (Exception ex) {
			Logger.getLogger(ClusterServerHostingConfigServlet.class.getName()).log(Level.SEVERE, null, ex);
		}
	}
}
