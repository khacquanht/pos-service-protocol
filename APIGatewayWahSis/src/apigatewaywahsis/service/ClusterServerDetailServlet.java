...
import com.service_wahsis.database_service.ClusterServerDetailHandler;
 ...
 * @author khiemnv
*/
public class ClusterServerDetailServlet extends AbstractServlet{
	@Override
	protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
		...
		String data = req.getParameter(DefinedName.PARAM_DATA);
		 if (cmd != null && data != null)
			ClusterServerDetailHandler.getInstance().process(cmd, data, respContent, vrfContent);
		 else
			respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
		} catch (Exception ex) {
			Logger.getLogger(ClusterServerDetailServlet.class.getName()).log(Level.SEVERE, null, ex);
		}
	}
}
