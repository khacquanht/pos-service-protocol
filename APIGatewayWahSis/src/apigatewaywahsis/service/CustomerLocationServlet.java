...
import com.service_wahsis.customer_service.CustomerLocationHandler;
 ...
 * @author khiemnv
 */
public class CustomerLocationServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                CustomerLocationHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(CustomerLocationServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
