...
import com.service_wahsis.master_service.MasterGroupHandler;
 ...
 * @author khiemnv
 */
public class MasterGroupServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                MasterGroupHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(MasterGroupServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
