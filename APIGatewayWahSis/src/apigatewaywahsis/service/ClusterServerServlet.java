...
import com.service_wahsis.database_service.ClusterServerHandler;
 ...
 * @author khiemnv
*/
public class ClusterServerServlet extends AbstractServlet{
	@Override
	protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
		...
		String data = req.getParameter(DefinedName.PARAM_DATA);
		 if (cmd != null && data != null)
			ClusterServerHandler.getInstance().process(cmd, data, respContent, vrfContent);
		 else
			respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
		} catch (Exception ex) {
			Logger.getLogger(ClusterServerServlet.class.getName()).log(Level.SEVERE, null, ex);
		}
	}
}
