...
import com.service_wahsis.company_service.CompanyDevicesHandler;
 ...
 * @author khiemnv
*/
public class CompanyDevicesServlet extends AbstractServlet{
	@Override
	protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
		...
		String data = req.getParameter(DefinedName.PARAM_DATA);
		 if (cmd != null && data != null)
			CompanyDevicesHandler.getInstance().process(cmd, data, respContent, vrfContent);
		 else
			respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
		} catch (Exception ex) {
			Logger.getLogger(CompanyDevicesServlet.class.getName()).log(Level.SEVERE, null, ex);
		}
	}
