...
import com.service_wahsis.report_service.ReportInfoHandler;
 ...
 * @author khiemnv
 */
public class ReportInfoServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                ReportInfoHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(ReportInfoServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
