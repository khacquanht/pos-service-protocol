...
import com.service_wahsis.coordinator.CoordinatorHandler;
 ...
 * @author khiemnv
 */
public class CoordinatorServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                CoordinatorHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(CoordinatorServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
}
