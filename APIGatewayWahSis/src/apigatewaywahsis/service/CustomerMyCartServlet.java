...
import com.service_wahsis.customer_my_cart.CustomerMyCartHandler;
 ...
 * @author khiemnv
*/
public class CustomerMyCartServlet extends AbstractServlet{
	@Override
	protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
		...
		String data = req.getParameter(DefinedName.PARAM_DATA);
		 if (cmd != null && data != null)
			CustomerMyCartHandler.getInstance().process(cmd, data, respContent, vrfContent);
		 else
			respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
		} catch (Exception ex) {
			Logger.getLogger(CustomerMyCartServlet.class.getName()).log(Level.SEVERE, null, ex);
		}
	}
}
