...
import com.service_wahsis.utility_service.MailingQueueHandler;
 ...
 * @author khiemnv
 */
public class MailingQueueServlet extends AbstractServletNonAuthenticate {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                MailingQueueHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(MailingQueueServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
