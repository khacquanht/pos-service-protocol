...
import com.service_wahsis.company_service.CompanyWorkingScheduleHandler;
 ...
 * @author khiemnv
 */
public class CompanyWorkingScheduleServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                CompanyWorkingScheduleHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(CompanyWorkingScheduleServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
