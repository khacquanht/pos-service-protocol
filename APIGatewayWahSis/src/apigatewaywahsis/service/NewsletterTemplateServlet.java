...
import com.service_wahsis.utility_service.NewsletterTemplateHandler;
 ...
 * @author khiemnv
 */
public class NewsletterTemplateServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                NewsletterTemplateHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(NewsletterTemplateServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
