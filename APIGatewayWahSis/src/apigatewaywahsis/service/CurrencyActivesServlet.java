...
import com.service_wahsis.master_service.CurrencyActivesHandler;
 ...
 * @author khiemnv
 */
public class CurrencyActivesServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                CurrencyActivesHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(CurrencyActivesServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
