...
import com.service_wahsis.user_service.UsersRolesHandler;
 ...
 * @author khiemnv
 */
public class UsersRolesServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                UsersRolesHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(UsersRolesServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
