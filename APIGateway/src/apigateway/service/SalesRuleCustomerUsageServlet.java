...
import com.wahsis.promotion_service.SalesRuleCustomerUsageHandler;
 ...
 * @author khiemnv
 */
public class SalesRuleCustomerUsageServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                SalesRuleCustomerUsageHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(SalesRuleCustomerUsageServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
//	@Override
