...
import com.wahsis.credit_memo.SalesCreditMemoHandler;
 ...
 * @author khiemnv
 */
public class SalesCreditMemoServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                SalesCreditMemoHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(SalesCreditMemoServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
