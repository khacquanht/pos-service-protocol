...
import com.wahsis.promotion_service.CatalogRuleHandler;
 ...
 * @author khiemnv
 */
public class CatalogRuleServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                CatalogRuleHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(CatalogRuleServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
