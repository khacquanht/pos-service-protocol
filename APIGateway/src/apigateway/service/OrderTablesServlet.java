...
import com.wahsis.order_service.OrderTablesHandler;
 ...
 * @author khiemnv
 */
public class OrderTablesServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                OrderTablesHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(OrderTablesServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
