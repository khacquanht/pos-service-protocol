...
import com.wahsis.order_service.OrderStatusHandler;
 ...
 * @author khiemnv
 */
public class OrderStatusServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                OrderStatusHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(OrderStatusServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
