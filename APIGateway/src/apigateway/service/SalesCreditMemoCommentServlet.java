...
import com.wahsis.credit_memo.SalesCreditMemoCommentHandler;
 ...
 * @author khiemnv
 */
public class SalesCreditMemoCommentServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                SalesCreditMemoCommentHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(SalesCreditMemoCommentServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
