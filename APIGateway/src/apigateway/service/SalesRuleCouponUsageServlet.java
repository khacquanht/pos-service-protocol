...
import com.wahsis.promotion_service.SalesRuleCouponUsageHandler;
 ...
 * @author khiemnv
 */
public class SalesRuleCouponUsageServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                SalesRuleCouponUsageHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(SalesRuleCouponUsageServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
//	@Override
