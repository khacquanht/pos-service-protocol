...
import com.wahsis.customer_service.CustomerTypesHandler;
 ...
 * @author khiemnv
*/
public class CustomerTypesServlet extends AbstractServlet{
	@Override
	protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
		...
		String data = req.getParameter(DefinedName.PARAM_DATA);
		 if (cmd != null && data != null)
			CustomerTypesHandler.getInstance().process(cmd, data, respContent, vrfContent);
		 else
			respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
		} catch (Exception ex) {
			Logger.getLogger(CustomerTypesServlet.class.getName()).log(Level.SEVERE, null, ex);
		}
	}
	@Override
