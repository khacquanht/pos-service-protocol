...
import com.wahsis.sales_order_status.SalesOrderStatusHandler;
 ...
 * @author khiemnv
 */
public class SalesOrderStatusServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                SalesOrderStatusHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(SalesOrderStatusServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
