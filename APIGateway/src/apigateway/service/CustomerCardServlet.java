...
import com.wahsis.member_service.CustomerCardHandler;
 ...
 * @author khiemnv
 */
public class CustomerCardServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                CustomerCardHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(CustomerCardServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
//	@Override
