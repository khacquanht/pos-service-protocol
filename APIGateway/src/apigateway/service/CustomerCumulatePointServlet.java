...
import com.wahsis.customer_service.CustomerCumulatePointHandler;
 ...
 * @author khiemnv
 */
public class CustomerCumulatePointServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                CustomerCumulatePointHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(CustomerCumulatePointServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
//	@Override
