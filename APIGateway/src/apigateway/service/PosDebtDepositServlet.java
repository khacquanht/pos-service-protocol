...
import com.wahsis.pos_debt_deposit_service.PosDebtDepositHandler;
 ...
 * @author khiemnv
 */
public class PosDebtDepositServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                PosDebtDepositHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(PosDebtDepositServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
