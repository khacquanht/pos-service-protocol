...
import com.wahsis.export_item_service.ItemStockOutsHandler;
 ...
 * @author khiemnv
 */
public class ItemStockOutsServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                ItemStockOutsHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(ItemStockOutsServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
