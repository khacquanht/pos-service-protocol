...
import com.wahsis.core_service.TableRateConditionHandler;
 ...
 * @author khiemnv
 */
public class TableRateConditionServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                TableRateConditionHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(TableRateConditionServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
//	@Override
