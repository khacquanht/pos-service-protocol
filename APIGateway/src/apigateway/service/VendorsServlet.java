...
import com.wahsis.master_service.VendorsHandler;
 ...
 * @author khiemnv
 */
public class VendorsServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                VendorsHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(VendorsServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
