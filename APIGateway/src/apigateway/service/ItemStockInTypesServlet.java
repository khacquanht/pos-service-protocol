...
import com.wahsis.import_item_service.ItemStockInTypesHandler;
 ...
 * @author khiemnv
 */
public class ItemStockInTypesServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                ItemStockInTypesHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(ItemStockInTypesServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
