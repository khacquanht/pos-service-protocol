...
import com.wahsis.company_service.CompanyTableStaffHandler;
 ...
 * @author khiemnv
 */
public class CompanyTableStaffServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                CompanyTableStaffHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(CompanyTableStaffServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
