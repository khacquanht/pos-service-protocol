...
import com.wahsis.member_service.CumulativePointFormulaHandler;
 ...
 * @author khiemnv
 */
public class CumulativePointFormulaServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                CumulativePointFormulaHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(CumulativePointFormulaServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
