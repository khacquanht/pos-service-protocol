...
import com.wahsis.sales_invoice.SalesInvoiceCommentHandler;
 ...
 * @author khiemnv
 */
public class SalesInvoiceCommentServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                SalesInvoiceCommentHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(SalesInvoiceCommentServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
