...
import com.wahsis.delivery_service.DeliveryDateSettingHandler;
 ...
 * @author khiemnv
 */
public class DeliveryDateSettingServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                DeliveryDateSettingHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(DeliveryDateSettingServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
