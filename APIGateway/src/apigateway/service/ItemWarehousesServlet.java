...
import com.wahsis.items_service.ItemWarehousesHandler;
 ...
 * @author khiemnv
 */
public class ItemWarehousesServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                ItemWarehousesHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(ItemWarehousesServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
