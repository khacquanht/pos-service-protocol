...
import com.wahsis.company_service.CompanyPaymentMethodsHandler;
 ...
 * @author khiemnv
 */
public class CompanyPaymentMethodsServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                CompanyPaymentMethodsHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(CompanyPaymentMethodsServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
