...
import com.wahsis.delivery_service.DeliveryMethodsHandler;
 ...
 * @author khiemnv
 */
public class DeliveryMethodsServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                DeliveryMethodsHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(DeliveryMethodsServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
