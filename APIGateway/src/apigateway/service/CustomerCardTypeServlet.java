...
import com.wahsis.member_service.CustomerCardTypeHandler;
 ...
 * @author khiemnv
 */
public class CustomerCardTypeServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                CustomerCardTypeHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(CustomerCardTypeServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
