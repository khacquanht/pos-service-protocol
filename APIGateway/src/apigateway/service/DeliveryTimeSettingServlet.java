...
import com.wahsis.delivery_service.DeliveryTimeSettingHandler;
 ...
 * @author khiemnv
 */
public class DeliveryTimeSettingServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                DeliveryTimeSettingHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(DeliveryTimeSettingServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
