...
import com.wahsis.member_service.RedeemPointFormulaHandler;
 ...
 * @author khiemnv
 */
public class RedeemPointFormulaServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                RedeemPointFormulaHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(RedeemPointFormulaServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
