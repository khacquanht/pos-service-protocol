...
import com.wahsis.private_camera_service.MasterPrivateCameraHandler;
 ...
 * @author khiemnv
*/
public class MasterPrivateCameraServlet extends AbstractServlet{
	@Override
	protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
		...
		String data = req.getParameter(DefinedName.PARAM_DATA);
		 if (cmd != null && data != null)
			MasterPrivateCameraHandler.getInstance().process(cmd, data, respContent, vrfContent);
		 else
			respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
		} catch (Exception ex) {
			Logger.getLogger(MasterPrivateCameraServlet.class.getName()).log(Level.SEVERE, null, ex);
		}
	}
}
