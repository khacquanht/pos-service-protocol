...
import com.wahsis.export_item_service.ItemStockOutDetailsHandler;
 ...
 * @author khiemnv
 */
public class ItemStockOutDetailsServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                ItemStockOutDetailsHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(ItemStockOutDetailsServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
