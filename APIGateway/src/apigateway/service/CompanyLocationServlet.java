...
import com.wahsis.company_service.CompanyLocationHandler;
 ...
 * @author khiemnv
 */
public class CompanyLocationServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                CompanyLocationHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(CompanyLocationServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
