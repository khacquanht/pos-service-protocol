...
import com.wahsis.staff_service.EmployeesDepartmentHandler;
 ...
 * @author khiemnv
 */
public class EmployeesDepartmentServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                EmployeesDepartmentHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(EmployeesDepartmentServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
