...
import com.wahsis.promotion_service.SalesRuleHandler;
 ...
 * @author khiemnv
 */
public class SalesRuleServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                SalesRuleHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(SalesRuleServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
