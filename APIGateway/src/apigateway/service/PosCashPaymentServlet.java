...
import com.wahsis.pos_cash_service.PosCashPaymentHandler;
 ...
 * @author khiemnv
 */
public class PosCashPaymentServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                PosCashPaymentHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(PosCashPaymentServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
