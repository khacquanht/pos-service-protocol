...
import com.wahsis.shipment_service.SalesShipmentHandler;
 ...
 * @author khiemnv
 */
public class SalesShipmentServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                SalesShipmentHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(SalesShipmentServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
