...
import com.wahsis.shipment_service.SalesShipmentCommentHandler;
 ...
 * @author khiemnv
 */
public class SalesShipmentCommentServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                SalesShipmentCommentHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(SalesShipmentCommentServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
//
//    @Override
