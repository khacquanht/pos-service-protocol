...
import com.wahsis.company_service.CompanyReviewHandler;
 ...
 * @author khiemnv
 */
public class CompanyReviewServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                CompanyReviewHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(CompanyReviewServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
