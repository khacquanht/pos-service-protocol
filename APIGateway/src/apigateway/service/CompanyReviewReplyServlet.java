...
import com.wahsis.company_service.CompanyReviewReplyHandler;
 ...
 * @author khiemnv
 */
public class CompanyReviewReplyServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                CompanyReviewReplyHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(CompanyReviewReplyServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
