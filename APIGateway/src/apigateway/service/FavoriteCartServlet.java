...
import com.wahsis.shopping_cart.FavoriteCartHandler;
 ...
 * @author khiemnv
 */
public class FavoriteCartServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                FavoriteCartHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(FavoriteCartServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
