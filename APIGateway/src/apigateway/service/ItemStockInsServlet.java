...
import com.wahsis.import_item_service.ItemStockInsHandler;
 ...
 * @author khiemnv
 */
public class ItemStockInsServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                ItemStockInsHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(ItemStockInsServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
