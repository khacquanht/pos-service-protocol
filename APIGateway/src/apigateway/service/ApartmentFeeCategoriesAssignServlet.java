...
import com.wahsis.apartment_fee_service.ApartmentFeeCategoriesAssignHandler;
 ...
 * @author khiemnv
*/
public class ApartmentFeeCategoriesAssignServlet extends AbstractServlet{
	@Override
	protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
		...
		String data = req.getParameter(DefinedName.PARAM_DATA);
		 if (cmd != null && data != null)
			ApartmentFeeCategoriesAssignHandler.getInstance().process(cmd, data, respContent, vrfContent);
		 else
			respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
		} catch (Exception ex) {
			Logger.getLogger(ApartmentFeeCategoriesAssignServlet.class.getName()).log(Level.SEVERE, null, ex);
		}
	}
}
