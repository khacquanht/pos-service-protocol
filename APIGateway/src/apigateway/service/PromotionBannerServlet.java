...
import com.wahsis.promotion_service.PromotionBannerHandler;
 ...
 * @author khiemnv
 */
public class PromotionBannerServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                PromotionBannerHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(PromotionBannerServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
