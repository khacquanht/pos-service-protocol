...
import com.wahsis.inventory_service.ItemStocksHandler;
 ...
 * @author khiemnv
 */
public class ItemStocksServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                ItemStocksHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(ItemStocksServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
