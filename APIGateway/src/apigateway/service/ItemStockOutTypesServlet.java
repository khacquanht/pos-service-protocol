...
import com.wahsis.export_item_service.ItemStockOutTypesHandler;
 ...
 * @author khiemnv
 */
public class ItemStockOutTypesServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                ItemStockOutTypesHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(ItemStockOutTypesServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
