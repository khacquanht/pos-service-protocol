...
import com.wahsis.items_service.ItemPurchasePriceHandler;
 ...
 * @author khiemnv
 */
public class ItemPurchasePriceServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                ItemPurchasePriceHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(ItemPurchasePriceServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
