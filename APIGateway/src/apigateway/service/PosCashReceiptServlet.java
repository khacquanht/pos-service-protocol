...
import com.wahsis.pos_cash_service.PosCashReceiptHandler;
 ...
 * @author khiemnv
 */
public class PosCashReceiptServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                PosCashReceiptHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(PosCashReceiptServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
