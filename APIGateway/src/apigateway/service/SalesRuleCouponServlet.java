...
import com.wahsis.promotion_service.SalesRuleCouponHandler;
 ...
 * @author khiemnv
 */
public class SalesRuleCouponServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                SalesRuleCouponHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(SalesRuleCouponServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
