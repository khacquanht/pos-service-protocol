...
import com.wahsis.company_service.CompanyCommentReplyHandler;
 ...
 * @author khiemnv
 */
public class CompanyCommentReplyServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                CompanyCommentReplyHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(CompanyCommentReplyServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
//	@Override
