...
import com.wahsis.log_database_service.LogDatabaseHandler;
 ...
 * @author khiemnv
*/
public class LogDatabaseServlet extends AbstractServlet{
	@Override
	protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
		...
		String data = req.getParameter(DefinedName.PARAM_DATA);
		 if (cmd != null && data != null)
			LogDatabaseHandler.getInstance().process(cmd, data, respContent, vrfContent);
		 else
			respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
		} catch (Exception ex) {
			Logger.getLogger(LogDatabaseServlet.class.getName()).log(Level.SEVERE, null, ex);
		}
	}
}
