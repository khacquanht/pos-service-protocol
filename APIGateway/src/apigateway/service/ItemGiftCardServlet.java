...
import com.wahsis.items_service.ItemGiftCardHandler;
 ...
 * @author khiemnv
 */
public class ItemGiftCardServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                ItemGiftCardHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(ItemGiftCardServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
