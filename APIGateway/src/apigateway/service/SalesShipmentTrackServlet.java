...
import com.wahsis.shipment_service.SalesShipmentTrackHandler;
 ...
 * @author khiemnv
 */
public class SalesShipmentTrackServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                SalesShipmentTrackHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(SalesShipmentTrackServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
