...
import com.wahsis.shift_service.TransferShiftHandler;
 ...
 * @author khiemnv
 */
public class TransferShiftServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                TransferShiftHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(TransferShiftServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
