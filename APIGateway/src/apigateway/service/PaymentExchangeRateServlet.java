...
import com.wahsis.master_service.PaymentExchangeRateHandler;
 ...
 * @author khiemnv
 */
public class PaymentExchangeRateServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                PaymentExchangeRateHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(ExchangeRateServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
