...
import com.wahsis.member_service.CustomerCumulativeRedeemPointHandler;
 ...
 * @author khiemnv
 */
public class CustomerCumulativeRedeemPointServlet extends AbstractServlet {
    ...
    protected void processs(HttpServletRequest req, HttpServletResponse resp, StringBuilder respContent) {
        try {
            ...
            if (cmd != null && data != null) {
                CustomerCumulativeRedeemPointHandler.getInstance().process(cmd, data, respContent, vrfContent);
            } else {
                respContent.append(CommonModel.toJSON(-1, DefinedName.RESP_MSG_INVALID_REQUEST));
            }
        } catch (Exception ex) {
            Logger.getLogger(CustomerCumulativeRedeemPointServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
